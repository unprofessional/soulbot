...
module.exports = {
    async handleButton(interaction) {
        ...
        if (customId.startsWith('editGameModal:') || customId.startsWith('publishGame:')) return gameButtons.handle(interaction);
        if (customId.startsWith('defineStats:') || customId.startsWith('edit_stat_template:') || customId.startsWith('finishStatSetup:')) return statButtons.handle(interaction);
        if (customId === 'submitNewCharacter') return charCreateButtons.handle(interaction);
        if (customId.startsWith('edit_stat:')) return charEditButtons.handle(interaction);
        if (

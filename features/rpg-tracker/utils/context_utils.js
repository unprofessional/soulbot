...
} = require('../../../store/services/player_interaction.service');

/**
 * Persist the interaction context (i.e. a message ID for a player + game).
 * @param {string} playerId - UUID from `player` table
 * @param {string} gameId - Game ID
 * @param {string} messageId - Discord message ID
 * @param {string} [interactionType] - Defaults to 'game_stat_template'
 */
async function persistInteractionContext(playerId, gameId, messageId, interactionType = 'game_stat_template') {
    return saveInteraction({ playerId, gameId, messageId, interactionType });
}
 ...
 * @param {object} interaction - Discord interaction (used to resolve channel + player)
 * @param {string} playerId - UUID from `player` table
 * @param {string} gameId
 * @param {string} [interactionType]
 * @returns {Promise<import('discord.js').Message|null>}
 */
async function getOriginalTrackedMessage(interaction, playerId, gameId, interactionType = 'game_stat_template') {
    const messageId = await getRecentInteractionMessage({ playerId, gameId, interactionType });
 ...
 * @param {string} [interactionType]
 */
async function updateTrackedMessageOrReply(interaction, playerId, gameId, newEmbed, newComponents, interactionType = 'game_stat_template') {
    const trackedMessage = await getOriginalTrackedMessage(interaction, playerId, gameId, interactionType);
